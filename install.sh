...
engine_menu()
{
    ...
    case "$choice" in
        ...
        3)
            ...
            echo
            log_warn "Sing-box may consume more RAM on low-end devices."
            echo
            printf "  ⁉️ Continue? [y/N] : "
            read -r confirm </dev/tty

            case "$confirm" in
                y|Y) add_selected_package "sing-box" ;;
                *)   SELECTED_ENGINE="auto" ;;
            esac
            ;;
        *)
...
review_install()
{
    ...
    echo "  ⚙️ Selected Engine   : ${SELECTED_ENGINE:-auto}"
    echo "  🗣️ Language          : ${SELECTED_LANGUAGE:-none}"
